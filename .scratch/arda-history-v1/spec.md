...
### Spatial substrate
- **Two-layer hybrid.** A **region graph** (~50–100 named polygons: terrain, adjacency, `owner_faction_id`, `seat_location_id`, `base_yield`) carries politics/territory; a **location + route graph** (settlements/fords/passes/gates joined by canonical roads & rivers) carries movement and battle siting. Armies and the Ring hop node→node; they never free-roam. *(Tickets 01/06/08)*
- **All geometry stored in v7-image pixel coordinates**, with one **pixels-per-mile** calibration from the map's scale bar; movement budgeted in **miles/year** (distance sets how many ticks a journey takes). Geometry is fixed **config**, not run state. *(Ticket 01)*
- **Terrain hand-authored** per region (`plains/forest/mountain/marsh/hills/barren/sea`) and per route (`road/river/pass/open/forest`); impassability falls out of terrain + absence of a route. *(Ticket 01)*
- **World extent: the tightened War-of-the-Ring theatre** (Eriador → Erebor/Dale → Gondor → Mordor). Far north and deep Harad are static backdrop. Off-map peoples attach as **abstract provider nodes at edge gateways** (Harad Road/Poros, E. Rhovanion, SE Nurn/Mordor, Umbar-sea). *(Ticket 01/06)*
...
- **Marriage seam:** 08 decides *whether* a marriage happens (dynastic-alliance utility off disposition + eligible unwed heirs); 05 defines *what it does* (kinship edges, succession). *(Ticket 08/05)*
